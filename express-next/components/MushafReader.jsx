...
const MushafReader = () => {
  ...
  const handleNextPage = useCallback(() => {
    if (currentPage < 604) router.push(`/quran/mushaf/${currentPage + 1}`);
  }, [currentPage, router]);

  const handlePrevPage = useCallback(() => {
    if (currentPage > 1) router.push(`/quran/mushaf/${currentPage - 1}`);
  }, [currentPage, router]);
          ...
          {pageData?.ayahs?.length > 0 && !loading && (
            <div className="mushaf-page-indicators">
              ...
              <div className="indicator-group center-stamp">
                <img
                  src={`/api/v1/assets/fonts/Tuluth/Vector-${primarySurah - 1}.svg`}
