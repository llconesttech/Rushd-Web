...
const SettingsSidebar = ({ persistent = false }) => {
  ...
  const isMushafRoute = /^\/quran\/mushaf\/\d+$/.test(pathname);
  ...
  const [activeSection, setActiveSection] = useState(
    isShanENuzool ? "translations" : "quran",
  );
      ...
      <div className="settings-content">
        ...
        {activeSection === "quran" && (
          <section className="settings-section">
            ...
            <ul className="settings-list">
              {Object.entries(quranScripts).map(([key, value]) => (
                ...
                >
                  ...
                  {value.native_name && (
                    <span className="native-name">{value.native_name}</span>
                  )}
                </li>
              ))}
            </ul>
                ...
                {isIndoPak ? (
                  ...
                  <div className="font-selector-grid">
                    {fontOptions.map((font) => (
                      ...
                      >
                        <span className="font-arabic-label">
                          {font.label}
                        </span>
                        <span className="font-english-label">
                          {font.english}
                        </span>
                      </button>
                    ))}
            ...
            {/* Mushaf route: UI style toggle not applicable */}
            {!isMushafRoute && (
              <>
        ...
        {(activeSection === "translations" || activeSection === "tafsir") && (
          <section className="settings-section">
            ...
            <div className="translations-grouped">
              {(() => {
                const filteredItems = Object.entries(translations).filter(
                  ([, value]) => {
                    const matchesSearch =
                      value.english_name
                        .toLowerCase()
                      ...
                      value.native_name
                        ?.toLowerCase()
                        .includes(searchTerm.toLowerCase()) ||
                      languageList[value.language_code]?.english_name
                        ?.toLowerCase()
                        .includes(searchTerm.toLowerCase());
                ...
                if (filteredItems.length === 0) {
                  return (
                    <div className="no-results">No {activeSection} found.</div>
                  );
                }
                        ...
                        {group.items.map((item) => (
                          <li
                            ...
                            className={`settings-item ${
                              (() => {
                                if (activeSection === "tafsir")
                                  return selectedTafsir === item.key;
                            ...
                            onClick={() => {
                              if (activeSection === "tafsir") {
                                setSelectedTafsir(
                                  selectedTafsir === item.key
                                    ? "none"
                                    : item.key,
                                );
                              } else {
                                setSelectedTranslation(
                                  selectedTranslation === item.key
                                    ? "none"
                                    : item.key,
                                );
                              }
            ...
            <ul className="settings-list">
              {Object.entries(shanENuzoolList).map(([key, value]) => (
                <li
                  ...
                  onClick={() =>
                    setSelectedShanENuzool(
                      selectedShanENuzool === key ? "none" : key,
                    )
                  }
                >
                  <span className="item-name">{value.english_name}</span>
