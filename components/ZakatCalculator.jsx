...
const NISAB_VALUES = {
  gold: {
    ...
    label: "Gold Standard",
    sub: "87.48g",
    desc: "Use if your wealth is mostly gold.",
  },
  silver: {
    grams: 612.36,
    label: "Silver Standard",
    sub: "612.36g",
    desc: "Recommended: Safer for ensuring Zakat is paid.",
  },
};
...
const ZakatCalculator = () => {
  ...
  // 1. Money
  const moneyTotal =
    getVal("money_cashInHand") +
    getVal("money_cashInBank") +
    getVal("money_deposited");
  ...
  const investmentsTotal =
    getVal("investments_stocks") +
    getVal("investments_mutualFunds") +
    getVal("investments_profits");

  // 4. Properties
  const propertyTotal =
    getVal("property_investmentProperty") + getVal("property_rentalIncome");

  // 5. Business
  const businessTotal =
    getVal("business_stockInTrade") + getVal("business_receivables");

  // 6. Agriculture (Special Rates)
  const agriRate = () => {
    const type = values["agriculture_irrigationType"];
    if (type === "rain") return 0.1;
  ...
  const cattleZakat =
    getCattleHeadCount(cattleCount) * getVal("cattle_pricePerAnimal");
  ...
  // 9. Others
  const othersTotal =
    getVal("others_loansGiven") + getVal("others_otherAssets");
  ...
  const liabilitiesTotal =
    getVal("liabilities_debts") + getVal("liabilities_expenses");
              ...
              <div className="config-label">
                ...
                <InfoTooltip
                  text={
                    <>
                      <p>
                        <strong>Silver Standard:</strong> Lower threshold (~$
                        {(
                          NISAB_VALUES.silver.grams * metalPrices.silver
                        ).toFixed(0)}
                        ), safer to ensure obligation is met.
                      </p>
                      <div style={{ height: 4 }}></div>
                      <p>
                        <strong>Gold Standard:</strong> Higher threshold (~$
                        {(NISAB_VALUES.gold.grams * metalPrices.gold).toFixed(
                          0,
                        )}
                        ), used if wealth is purely gold.
                      </p>
                    </>
                  }
                />
        ...
        <div className="zakat-main-layout">
          ...
          <div className="zakat-cards-column">
            {ZAKAT_CATEGORIES.map((category) => (
              <ZakatCategoryCard
                ...
                values={Object.keys(values)
                  .filter((k) => k.startsWith(category.id))
                  .reduce((obj, k) => {
            ...
            <ZakatCategoryCard
              ...
              values={Object.keys(values)
                .filter((k) => k.startsWith("liabilities"))
                .reduce((obj, k) => {
            ...
            <div className="summary-sticky-wrapper">
              <div className="zakat-summary-card">
                <div className="summary-header">
                  <h3 className="summary-header-heading">
                    <span className="summary-header-icon" aria-hidden="true">
                      <Calculator size={20} strokeWidth={2} />
                    </span>
                    <span className="summary-header-titles">
                      <span className="summary-header-title">Summary</span>
                      <span className="summary-header-subtitle">
                        Updates as you enter assets
                      </span>
                    </span>
                  </h3>
                </div>
                ...
                <div className="summary-content">
                  <div className="summary-wealth-panel">
                    <div className="summary-item">
                      <span className="label">Zakatable assets</span>
                      <span className="value summary-value--tabular">
                        {formatCurrency(zakatableAssets)}
                      </span>
                    </div>
                  ...
                  <div className="nisab-indicator">
                    <div className="nisab-info">
                      ...
                      <span className="amount summary-value--tabular">
                  ...
                  {(agriZakat > 0 || cattleZakat > 0) && (
                    <div className="summary-divider" role="presentation" />
                  )}
