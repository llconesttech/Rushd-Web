...
export default function QuranReader() {
  ...
  if (error)
    return (
      <div className="container" style={{ marginTop: "2rem" }}>
        Error loading Surah. Try a different translation.
      </div>
    );
  if (!surah) return null;

  const isWordByWord =
    selectedScript === "quran-kids" ||
    selectedScript === "quran-wordbyword" ||
    selectedScript === "quran-wordbyword-2";
  const isTajweed =
  ...
  const handleNextSurah = () => {
    if (surahNum < 114) router.push(`/quran/${surahNum + 1}`);
  };
  const handlePrevSurah = () => {
    if (surahNum > 1) router.push(`/quran/${surahNum - 1}`);
  };

  const surahBadge = (
    <span
      className={`revelation-badge ${surahInfo?.revelationType.toLowerCase()}`}
    >
  ...
  const renderAyah = (ayah, index) => {
    const arabicContent = isWordByWord ? (
      ...
      <p
        className={`ayah-arabic arabic-text tajweed-text ${isIndoPak ? "indopak" : `font-${selectedArabicFont}`}`}
        style={{ fontFamily: getArabicFontFamily() }}
        dangerouslySetInnerHTML={{
          __html: parseTajweed(ayah.text, showTajweedTooltips),
        }}
      />
    ) : (
      <p
        className={`ayah-arabic arabic-text font-${selectedArabicFont} ${isIndoPak ? "indopak" : ""}`}
        style={{ fontFamily: getArabicFontFamily() }}
      >
        {ayah.text}
      </p>
        ...
        >
          ...
          <div className="ayah-text-container">
            ...
            {ayah.transliteration && (
              <p className="ayah-transliteration">{ayah.transliteration}</p>
            )}
            {ayah.translation && (
  ...
  return (
    <div
      ...
      style={{ "--arabic-font": getArabicFontFamily() }}
    >
      <div
        className="quran-header-group"
            ...
            >
              ...
              >
                <ReadingProgress
                  ...
                  onClearFilter={() => router.push(`/quran/${surahNum}`)}
                />
      ...
      >
        <Link
          href={`/quran/mushaf/${surah.ayahs[0]?.page || 1}`}
      ...
      <div className={`ayah-content-wrap ${isStyle2 ? "transparent" : ""}`}>
        ...
        {(pageFilter
          ? surah.ayahs.filter((a) => a.page === parseInt(pageFilter))
          : surah.ayahs
        ).map((ayah, index) => {
