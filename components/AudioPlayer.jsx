...
const AudioPlayer = ({ surahNumber, totalAyahs }) => {
  ...
  const localUrl = audioService.getLocalUrl(
    selectedReciter,
    surahNumber,
    currentAyah,
  );
  const remoteUrl = audioService.getRemoteUrl(
    selectedReciter,
    surahNumber,
    currentAyah,
  );
  ...
  useEffect(() => {
    ...
    // Never auto-play on initial page load. Only resume/play if the user has started playback.
    const tryPlay =
      hasStartedPlayback &&
  ...
  }, [currentAyah, currentSrc, hasStartedPlayback]);

  // Reset the player when the surah changes
  useEffect(() => {
    // Ensure we stop any audio immediately on navigation.
    const audio = audioRef.current;
    if (audio) {
      try {
        audio.pause();
  ...
  const handleError = () => {
    if (!remoteUrlUsed && localUrl && remoteUrl && localUrl !== remoteUrl) {
      ...
      setAudioError(null);
      // Only retry/resume automatically if user already started playback.
  ...
  return (
    <div className={`floating-player ${isExpanded ? "expanded" : ""}`}>
      ...
      <div className="fp-main">
        ...
        >
          <span className="fp-ayah-text">
            {currentAyah}
            <span className="fp-ayah-divider">/</span>
            {totalAyahs}
          </span>
          <ChevronDown
            size={12}
            className={`fp-expand-icon ${isExpanded ? "rotated" : ""}`}
          />
        </button>
        ...
        >
          ...
          <span className="fp-reciter-chip-name">
            {currentReciter?.english_name?.split(" ").slice(0, 2).join(" ") ||
              "Reciter"}
          </span>
        </button>
        ...
        <div className="fp-expanded-section">
          <div className="fp-reciter-row">
            ...
            >
              ...
              <ChevronDown
                size={12}
                className={showReciterMenu ? "rotated" : ""}
              />
            </button>
          </div>
      ...
      {audioError && <div className="fp-error">{audioError}</div>}
    </div>
  );
};
