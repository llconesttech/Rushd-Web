...
const HadithReader = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const translationHadiths = await getSectionHadiths(
          bookId,
          sectionId,
          selectedLang,
        );
        ...
        const name =
          edition.metadata?.sections?.[sectionId] || `Chapter ${sectionId} `;
        setSectionName(name);
      } catch (e) {
        setError(e.message);
      }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (
          langToSearch !== selectedLang &&
          langToSearch !== "ara" &&
          langToSearch !== "eng"
        ) {
          try {
            targetEdition = await getEdition(bookId, langToSearch);
          } catch (e) {
            /* ignore */
          }
        } else if (langToSearch === "eng" && selectedLang !== "eng") {
          try {
            targetEdition = await getEdition(bookId, "eng");
          } catch (e) {
            /* ignore */
          }
        }
        ...
        for (const h of displayEdition.hadiths) {
          ...
          const matchAra = araEdition.hadiths.find(
            (a) => a.hadithnumber === h.hadithnumber,
          );
          const araText = matchAra?.text || "";

          let targetText = "";
          if (targetEdition) {
            const matchTarget = targetEdition.hadiths.find(
              (t) => t.hadithnumber === h.hadithnumber,
            );
  ...
  const displayHadiths =
    searchResults !== null ? searchResults : localFilteredHadiths;
  ...
  function getArabicText(hadith) {
    ...
    const match = arabicHadiths.find(
    ...
    <div className="hadith-reader-container">
      ...
      <div className="hadith-reader-toolbar">
        ...
        <span className="hadith-count-label">
          ...
          {searchResults === null && displayHadiths.length !== hadiths.length
            ? ` / ${hadiths.length}`
            : ""}{" "}
          Hadiths
        </span>
      ...
      {isSearching && (
        <div className="hadith-loading">
          Searching globally across the book...
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                <div className="hadith-card-header">
                  ...
                  {gradeLabel && (
                    <span
                      className={`grade-badge ${getGradeClass(gradeLabel)}`}
                    >
                      {gradeLabel}
                    </span>
                  )}
                  {hadith.reference && (
                    <span className="hadith-ref">
                      Book {hadith.reference.book}, Hadith{" "}
